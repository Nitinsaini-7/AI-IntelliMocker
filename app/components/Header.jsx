...
return (
  <div className='shadow-md w-full fixed top-0 left-0 z-10'>
    ...
    <ul className={`md:flex md:items-center md:bg-transparent md:pb-0 pb-12 absolute md:static bg-gray-900 border lg:border-none md:z-auto z-[-1] left-0 w-full md:w-auto md:pl-0 pl-9 transition-all duration-500 ease-in ${open ? 'top-20':'top-[-490px]'}`}>
      {
        Links.map((link)=>(
          <li key={link.name} className='md:ml-8 text-md md:my-0 my-7'>
            <Link href={link.link} className='text-white hover:text-blue-400 duration-300'>{link.name}</Link>
          </li>
        ))
      }
    </ul>
